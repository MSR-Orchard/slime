...
def resolve_parser_type(name_or_path: str, override: str | None = None) -> str:
    ...
    Args:
        name_or_path: HF checkpoint path or repo id.
        override: Optional explicit parser type; "auto"/None triggers detection.
...
TOOL_INSTRUCTION = (
    ...
    "with task execution using <tool_call></tool_call> XML tags.\n"
...
class ToolParser:
    ...
    def __init__(self, tools_info: list[dict] | None = None, parser_type: str = "qwen25"):
        """
        ...
        print(f"Initialized ToolParser with parser_type='{self.parser_type}'...")
            ...
            try:
                ...
                if tools_list:
                    self._sglang_parser = FunctionCallParser(
                        tools=tools_list, tool_call_parser=self.parser_type
                    )
            except Exception as e:
                print(f"⚠️ Failed to initialize sglang tool parser ({e}); falling back to regex parsing.")
    ...
    def _parse_regex(self, response: str) -> ParsedToolResult:
        ...
        print("⚠️ Using regex-based tool parsing. Consider using a native tool calling format for better reliability.")
...
class TokenHandler:
    ...
    def apply_chat_template(
        ...
        Args:
            ...
            enable_thinking: Whether to let the chat template inject a
                ``<think>`` prefix into the generation prompt. Set to False
                (default) when the system prompt already instructs the model
                to produce ``<think>`` blocks, to avoid a duplicate/empty
                thinking block.
