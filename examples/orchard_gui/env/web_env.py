...
from examples.orchard_gui.env.base_env import BaseEnv
from examples.orchard_gui.env.utils.js import find_clickable_elements_js_code
from examples.orchard_gui.env.utils.keyboard import validate_and_correct_key
import logging
...
class WebEnv(BaseEnv):
    ...
    def __init__(self,
        ...
        Args:
            ...
            start_url: URL to open when the environment starts
        ...
        if resize_output_coords:
            if resize_scale <= 0:
                ...
                self.smart_resize_height, self.smart_resize_width = smart_resize(
                    height, width, factor=image_patch_size,
                )
            else:
    ...
    async def setup(self) -> None:
        ...
        logger.info(
            f"Initializing WebEnv, browser type: {self.browser_type.name}, proxy settings: {self.proxy_settings}, viewport size: {self.screen_size}, DPR: {self.dpr}, timeout: {self.timeout}s"
        )
    ...
    async def reset(self,
        ...
        # Give Cloudflare / interstitial challenges time to resolve before
        # capturing the initial observation.
        ...
        for url in start_urls:
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    # Best-effort wait for "load" so later page.evaluate calls
                    # don't race with in-flight client-side redirects.
                    try:
    ...
    async def _get_element_description(self, x: float, y: float) -> str:
        """Get a human-readable description of the DOM element at (x, y) coordinates."""
        try:
            info = await self.page.evaluate(
                """({x, y}) => {
                    ...
                    return { tag, role, type, ariaLabel, text, checked };
                }""",
                {"x": x, "y": y},
            )
            if not info:
            ...
            if info.get("checked") is not None and info["tag"] == "input" and info.get("type") in ("checkbox", "radio"):
    ...
    async def _get_focused_element_description(self) -> str:
        """Get a human-readable description of the currently focused DOM element."""
        try:
            info = await self.page.evaluate(
                """() => {
    ...
    def _get_element_signature(element: Dict[str, Any]) -> tuple:
        """Build a semantic key for an a11ytree element for diffing purposes."""
        tag = element.get("tag", "")
        etype = element.get("type", "") or ""
        text = (element.get("ariaLabel") or element.get("textContent") or element.get("innerText") or "").strip()
        if len(text) > 30:
            text = text[:30]
        return (tag, etype, text)

    def _diff_a11ytree(self, before: list, after: list) -> str:
        """Compare two a11ytree snapshots and return a summary of changes."""
        before_sigs = {}
        for el in before:
        ...
        if added:
            total = len(added)
            shown = [_fmt(s) for s in added[:5]]
            desc = f"{total} element(s) appeared [{', '.join(shown)}]"
            if total > 5:
                desc += f" and {total - 5} more"
            parts.append(desc)
        if removed:
            total = len(removed)
            shown = [_fmt(s) for s in removed[:5]]
            desc = f"{total} element(s) disappeared [{', '.join(shown)}]"
            if total > 5:
                desc += f" and {total - 5} more"
            parts.append(desc)
    ...
    async def _execute_click(self, parameters: Dict[str, Any]) -> tuple[bool, str]:
        ...
        try:
            ...
            # Identify target element before clicking
            ...
            # Build feedback with element info and state change info
            ...
            if post_tab_count > pre_tab_count:
                new_tab_index = post_tab_count - 1
                feedback += f" New tab opened (tab {new_tab_index})."
            ...
            return False, f"Failed: `click` at ({x:.0f}, {y:.0f}) execution failed: {e}"
    ...
    async def _execute_write(self, parameters: Dict[str, Any]) -> tuple[bool, str]:
        ...
        try:
            ...
            # Identify focused element before typing
            ...
            try:
                ...
                if actual_value and actual_value != message:
                    feedback += f" Note: the field's actual value is \"{actual_value}\", which differs from the typed text. The page may have reformatted or autocompleted the input."
            except Exception:
                pass
    ...
    async def _execute_scroll(self, parameters: Dict[str, Any]) -> tuple[bool, str]:
        ...
        try:
            ...
            # Build feedback with scroll position verification
            ...
            if abs(dx_actual) < 1 and abs(dy_actual) < 1 and not point_2d:
                feedback += " Note: page scroll position did not change — may have reached the boundary."
    ...
    async def _execute_press_keys(self, parameters: Dict[str, Any]) -> tuple[bool, str]:
        ...
        try:
            ...
            # Build feedback with state change info
    ...
    async def _execute_hover(self, parameters: Dict[str, Any]) -> tuple[bool, str]:
        ...
        try:
            ...
            # Identify target element before hovering
            elem_desc = await self._get_element_description(x, y)
    ...
    async def step(self, action_list: list[dict]):
        ...
        try:
            # Capture a11ytree before actions for diff
            ...
            # Capture a11ytree after actions and compute diff
            ...
            # Get new observation (reuse post_a11ytree to avoid redundant call)
            observation = {
                "screenshot": await self.get_screenshot(),
