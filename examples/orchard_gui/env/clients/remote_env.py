...
Server URL(s) come from ``remote.server_url`` in env/config.yaml:
    # Single server
    remote:
      server_url: "http://browser-env:8100"

    # Multiple servers (comma-separated)
    remote:
      server_url: "http://server-a:8100,http://server-b:8100"
"""
...
class RemoteWebEnv:
    ...
    async def initialize(
        ...
        """
        Send ``POST /reset`` to the server, creating the environment for
        *task_id*.  Caches the observation so that the next ``reset()`` call
        returns it immediately.

        All configuration is sent from the client — the server holds no local
        config/task files (mirrors ``SandboxWebEnv.initialize`` and the
        ``ResetRequest`` contract in ``docker_server/env_server.py``).
        """
        payload = {
            "task_id": task_id,
            "task_data": task_data,
            ...
            try:
                async with aiohttp.ClientSession(timeout=_HTTP_TIMEOUT) as session:
                    async with session.get(f"{url}/health") as resp:
                        ...
                        # docker_server/env_server.py is single-env and its /health
                        # does not report capacity; treat each such server as
                        # one slot. A multi-env server may advertise num_envs.
                        num_envs = data.get("num_envs", 1)
                        for i in range(num_envs):
                            self._available.put_nowait((url, i))
                        logger.info("Discovered %d env(s) on %s", num_envs, url)
            except Exception as exc:
