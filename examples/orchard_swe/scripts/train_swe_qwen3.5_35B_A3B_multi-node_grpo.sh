...
# Uses the GRPO advantage estimator with a fixed number of samples per prompt.
# See train_swe_qwen3.5_35B_A3B_multi-node_grpo_dynamic.sh for the progressive-rollout variant.
...
WANDB_GROUP_NAME="${MODEL_NAME}_pe${PPO_EPOCHS}_r${ROLLOUT_BATCH_SIZE}_n${N_SAMPLES}_g${GLOBAL_BATCH_SIZE}_${ADVANTAGE_ESTIMATOR}_lr${LR}_${NUM_NODES}nodes_${WANDB_RANDOM_SUFFIX}"


CKPT_SUBDIR="pe${PPO_EPOCHS}_r${ROLLOUT_BATCH_SIZE}_n${N_SAMPLES}_g${GLOBAL_BATCH_SIZE}_${ADVANTAGE_ESTIMATOR}_lr${LR}_${NUM_NODES}nodes_${RANDOM_SUFFIX}"
