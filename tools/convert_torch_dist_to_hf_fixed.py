    ...
    if origin_hf_dir is not None:
        ...
        for filename in safetensors_files:
            with safetensors.safe_open(os.path.join(origin_hf_dir, filename), framework="pt", device="cpu") as f:
                for k in f.keys():
                    # BUGFIX 1: skip weights already produced by the megatron->hf conversion.
                    if k in converted_names:
                        continue
                    # BUGFIX 2: skip MoE experts that already exist under a *split* name.
                    # The origin hf stores experts fused as "experts.gate_up_proj" /
                    # "experts.down_proj", but convert_to_hf writes them per-expert as
                    # "experts.{i}.{gate,up,down}_proj.weight". Without this check the fused
                    # tensors look "missing" and get re-added, duplicating all expert weights.
                    if re.search(r"\.mlp\.experts\.(gate_up_proj|down_proj)$", k):
                        expert_prefix = k.rsplit(".", 1)[0] + "."
                        if any(name.startswith(expert_prefix) for name in converted_names):
                            continue
                    ...
                    current_size += tensor_size
                    # BUGFIX 3: only count tensors that were actually added (this line used to
                    # live outside the "missing" branch, re-adding a stale tensor_size for every
                    # already-converted key and massively inflating total_size).
                    total_size += tensor_size
